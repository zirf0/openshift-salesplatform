...
class Vtiger_MassActionAjax_View extends Vtiger_IndexAjax_View {
    ...
    // SalesPlatform begin SPSocialConnector module
    /**
	 * Function shows form that will let you send messages to social nets
	 * @param Vtiger_Request $request
	 */
	function showSendMessageForm(Vtiger_Request $request) {
		...
		$user = Users_Record_Model::getCurrentUserModel();
        $moduleModel = Vtiger_Module_Model::getInstance($sourceModule);
        $urlFields = $moduleModel->getFieldsByType('url');
		...
		$viewer->assign('EXCLUDED_IDS', $excludedIds);
		$viewer->assign('USER_MODEL', $user);
		$viewer->assign('URL_FIELDS', $urlFields);
		...
		echo $viewer->view('SendMsgForm.tpl', $moduleName, true);
	}
    
    /**
	 * Function shows form that will let you enter url of client account in the social net
	 * @param Vtiger_Request $request
	 */
	function showEnterURLForm(Vtiger_Request $request) {
