					...
					<div class="box box-primary">
						<div class="box-body">
							...
							<div class="form-group">
								...
								<?php
									$list = '<select name=productid  class="form-control">';
									$list .= '<OPTION value="">'.getTranslatedString('NONE').'</OPTION>';
								    for($i=0;$i<count($productslist[0]);$i++){
									$list .= '<OPTION value="'.$productslist[0][$i].'" >'.$productslist[1][$i].'</OPTION>';
									}
									$list .= '</select>';
									echo $list;
								?>
							...
							<div class="box-footer">
	                            ...
	                            <button title="<?PHP echo getTranslatedString('LBL_SUBMIT_ALT');?>" accessKey="S" class="btn btn-primary" value="<?PHP echo getTranslatedString('LBL_SAVE');?>" onclick="return formvalidate(this.form)" type="submit" name="button">
                                <!-- <button title="<//?PHP echo getTranslatedString('LBL_SAVE_ALT');?>" accessKey="S" class="btn btn-primary" value="<//?PHP echo getTranslatedString('LBL_SAVE');?>" onclick="return formvalidate(this.form)" type="submit" name="button"> -->
                                    <?PHP echo getTranslatedString('Submit');?>
	                            	<!-- Submit -->
	                            ...
	                            <button title="<?PHP echo getTranslatedString('LBL_CANCEL_ALT');?>" accessKey="X" class="btn btn-primary" onclick="window.history.back()" type="button" name="button"  value="<?PHP echo getTranslatedString('LBL_CANCEL');?>">
                                    <!-- SalesPlatform.ru begin localization -->
                                    <?PHP echo getTranslatedString('Cancel');?>
                                    <!-- Cancel -->
                                    <!-- SalesPlatform.ru end -->
	                           	</button>
							</div>
...
function formvalidate(form)
{
	...
	{
        // SalesPlatform.ru begin localization
		alert("Заголовок Заявки не заполнен");
