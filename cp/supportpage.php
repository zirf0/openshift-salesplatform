...
function GetForgotPasswordUI($mail_send_message='')
{
    ...
    $list .= '<input type="hidden" name="param" value="forgot_password">';
        ...
        $list .= '</div>';
        // SalesPlatform.ru begin localization
		$list .= '<div class="footer"><button class="btn bg-light-blue btn-block" type="submit" value="" style="font-size:16px;font=weight:500;">'.getTranslatedString('LBL_SEND_PASSWORD').'</button><br/><a href = "login.php">'.getTranslatedString('Back To Login').'</a>';
...
if($_REQUEST['mail_send_message'] != '')
{
	...
	if($mail_send_message[0] == 'true')
	{
		...
		$list .= '<br><tr><td>&nbsp;</td></tr>';
        // SalesPlatform.ru begin localization
		$list .= '<tr><td class="dvtCellInfo">'.getTranslatedString('Mail has been sent to your mail id with the customer portal login details').'</td></tr>';
