						...
						{if $CHILD_COMMENTS_MODEL neq null and ($CHILDS_ROOT_PARENT_ID neq $PARENT_COMMENT_ID)}
							...
							<span class="viewThreadBlock" data-child-comments-count="{$CHILD_COMMENTS_COUNT}">
								<a class="cursorPointer viewThread">
                                    {* SalesPlatform.ru begin *}
									<span class="childCommentsCount">{$CHILD_COMMENTS_COUNT}</span>&nbsp;{if $CHILD_COMMENTS_COUNT eq 1}{vtranslate('LBL_ONE_REPLY',$MODULE_NAME)}{elseif $CHILD_COMMENTS_COUNT ge 5}{vtranslate('LBL_REPLIES',$MODULE_NAME)}{else}{vtranslate('LBL_SOME_REPLIES',$MODULE_NAME)}{/if}&nbsp;
