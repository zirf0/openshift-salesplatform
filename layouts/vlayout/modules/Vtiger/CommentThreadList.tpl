						...
						{if $CHILD_COMMENTS_COUNT neq 0}
							...
							<span class="hide viewThreadBlock" data-child-comments-count="{$COMMENT->getChildCommentsCount()}">
									<a class="cursorPointer viewThread">
                                        {* SalesPlatform.ru begin *}
                                        <span class="childCommentsCount">{$COMMENT->getChildCommentsCount()}</span>&nbsp;{if $CHILD_COMMENTS_COUNT eq 1}{vtranslate('LBL_ONE_REPLY',$MODULE_NAME)}{elseif $CHILD_COMMENTS_COUNT ge 5}{vtranslate('LBL_REPLIES',$MODULE_NAME)}{else}{vtranslate('LBL_SOME_REPLIES',$MODULE_NAME)}{/if}&nbsp;
